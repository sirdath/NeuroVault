...
def consolidate(
    ...
    # 8. Knowledge compilation — LLM-as-compiler loop that rewrites
    # canonical wiki pages from their raw sources. Gated behind the
    # NEUROVAULT_COMPILER_ENABLED env var so the scheduler is off by default
    # during the v1 rollout (we want the prompt to stabilize before it
    # runs unattended on every brain). Manual triggers via the API /
    # MCP tool work regardless of the env gate.
    from neurovault_server.config import env_with_legacy_fallback as _env
    if brain_ctx is not None and _env("NEUROVAULT_COMPILER_ENABLED", "ENGRAM_COMPILER_ENABLED") == "1":
        try:
            from neurovault_server.compiler import run_pending_compilations
            from neurovault_server.karpathy import append_log
            compiled = run_pending_compilations(brain_ctx, max_per_run=5)
            stats["compilations_written"] = len(compiled)
            if compiled:
                logger.info("compiler: wrote {} compilation(s) during consolidation cycle", len(compiled))
                try:
                    for c in compiled:
                        append_log(
                            brain_ctx.vault_dir,
                            "wiki_compiled",
                            f"{c.topic} · {len(c.changelog)} changes · model={c.model}",
                        )
                except Exception as log_e:
                    logger.debug("compiler: append_log failed: {}", log_e)
        except Exception as e:
            logger.debug("Knowledge compilation skipped: {}", e)
