...
logic lives in `neurovault_server.hooks` server-side, so we can iterate without
touching user config.

Usage (from a hook config):
    uv run python -m scripts.neurovault_hook --event PostToolUse
...
DEFAULT_URL = (
    os.environ.get("NEUROVAULT_API_URL")
    or os.environ.get("ENGRAM_API_URL")  # back-compat
