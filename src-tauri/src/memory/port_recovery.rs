...
//!
//! Sibling module to `http_server` and `api_gateway`; both call
//! `try_clear_stale_neurovault` before retrying their bind. Lives
//! outside the routers so the recovery logic can grow without
//! perturbing the trust-the-machine path.
...
/// process (left alone). Logs to stderr on every interesting state
/// transition.
...
/// Cross-platform listener-PID lookup. Walks every TCP socket in
/// LISTEN state on loopback + IPv6 and collects the PIDs whose
/// `local_port` matches.
fn find_listener_pids(port: u16) -> Result<Vec<u32>, String> {
...
mod tests {
    ...
    fn empty_when_port_unbound() {
        ...
        // return Ok(vec![]).
        let pids = find_listener_pids(1).unwrap_or_default();
